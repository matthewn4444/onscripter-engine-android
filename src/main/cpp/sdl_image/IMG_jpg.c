...
static int is_jpeg_format(SDL_Surface *surf)
{
...
{
    static SDL_PixelFormat jpg_format = { 	NULL, 24, 3,
											0, 0, 0, 8,
    ...
    if (!is_jpeg_format(surface)) {

        // Make a copy of our errant surface
        jpeg_surface = SDL_ConvertSurface(surface, &jpg_format, SDL_SWSURFACE);
        if (!jpeg_surface) {
            goto done;
        }
    }
